...
class PlayerPlane: SKSpriteNode {
    ...
    func performFly() {
        ...
        motionManager.startAccelerometerUpdates(to: OperationQueue.current!) { [unowned self] (data, error) in
            if let data = data {
                let accseleration = data.acceleration
                self.xAcceleration = CGFloat(accseleration.x) * 0.7 + self.xAcceleration * 0.3
        ...
        let planeSequeceForever = SKAction.repeatForever(planeSequence)
        self.run(planeSequeceForever)
    }
